...
class SGD(Optimizer):
    ...
    def apply_with_lr(self, epoch, lr, grad, value, name, step=-1):
        ...
        # TODO: Self-implement
        # 2. SGD Updates with momentum
        self.opt.Apply(epoch, lr, name, grad.singa_tensor, value.singa_tensor)

        return value

class BM_MD(Optimizer):
    '''The vallina Bath Manhattan algorithm with momentum.
    ...
    def __init__(self, lr=None, momentum=None, weight_decay=None,
                 regularizer=None, constraint=None):
        super(BM_MD, self).__init__(lr, momentum, weight_decay, regularizer,
                                  constraint)
        conf = model_pb2.OptimizerConf()
        if self.momentum is not None:
            conf.momentum = self.momentum
    ...
    def apply_with_lr(self, epoch, lr, grad, value, name, step=-1):
        ...
        # TODO: Self-implement
        # 3. BM Updates with momentum
        grad = tensor.sign(grad)
        self.opt.Apply(epoch, lr, name, grad.singa_tensor, value.singa_tensor)

        return value

class BM_Scale_MD(Optimizer):
    '''The Batch Manhattan algorithm scaled by a random matrix with momentum.
    ...
    def __init__(self, lr=None, momentum=None, weight_decay=None,
                 regularizer=None, constraint=None):
        super(BM_Scale_MD, self).__init__(lr, momentum, weight_decay, regularizer,
                                  constraint)
        conf = model_pb2.OptimizerConf()
        if self.momentum is not None:
            conf.momentum = self.momentum
    ...
    def apply_with_lr(self, epoch, lr, grad, value, name, step=-1):
        ...
        # Create two intermediate gradients tensor
        mid_grad = tensor.Tensor(grad.shape, grad.device, grad.dtype)
...
class BM_Adam(Optimizer):
    ...
    def __init__(self, beta_1=0.9, beta_2=0.999, epsilon=1e-8, lr=None,
        ...
        super(BM_Adam, self).__init__(lr, None, weight_decay, regularizer,
                                   constraint)
        self.beta_1 = beta_1
        self.beta_2 = beta_2
    ...
    def apply_with_lr(self, epoch, lr, grad, value, name, step):
        ...
        # TODO: Self-implement
        # Follow BM rules, get signs of gradients
        grad = tensor.sign(grad)
